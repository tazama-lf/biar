...
#c.JupyterHub.runtime_dir = "/srv/jupyterhub"
# ----------------------------
# Authentication (TEMP: simple)
# ----------------------------
import secrets
...
# Map all authenticated users to a single system user
c.Authenticator.admin_users = {'jupyterhub'}

# ----------------------------
# Notebook server behavior  
# ----------------------------
c.Spawner.notebook_dir = '/app/notebooks'
c.Spawner.default_url = '/lab'
c.Spawner.cmd = ['jupyter-labhub']

# Pass environment variables to spawned notebooks
...
# Pre-spawn hook to create user directories
async def create_user_dir(spawner):
    ...
    if not os.path.exists(user_dir):
        ...
        try:
            result = subprocess.run(['useradd', '-m', '-s', '/bin/bash', username], 
                          capture_output=True, check=False)
            # Ensure home directory has correct permissions
            subprocess.run(['chown', '-R', f'{username}:{username}', user_dir],
                          capture_output=True, check=False)
...
# Disable user config to simplify permissions
...
# Configure template and static files paths
...
# Configure proxy API port to avoid conflict with FastAPI on 8001
c.ConfigurableHTTPProxy.api_url = 'http://127.0.0.1:8099'
