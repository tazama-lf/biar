...
class ConditionsETL(BaseETL):
    ...
    def silver(self) -> str:
        ...
        condition_key_key = (
            F.when(target_type == "ENTITY",  F.concat(F.coalesce(ntty_id, F.lit("")), F.coalesce(ntty_scheme, F.lit(""))))
             .when(target_type == "ACCOUNT", F.concat(F.coalesce(acct_id, F.lit("")), F.coalesce(acct_scheme, F.lit("")), F.coalesce(acct_mmb_id, F.lit(""))))
             .otherwise(F.lit(None).cast("string"))
        )
        ...
        evt_tp_count = F.coalesce(F.size(F.from_json(evt_tp_json, T.ArrayType(T.StringType()))), F.lit(0)).cast("int")
