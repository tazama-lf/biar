...
class TypologiesETL(BaseETL):
    ...
    def silver(self) -> str:
        ...
        silver = (
            ...
            .withColumn("rule_count",                 F.size(F.col("typology_obj").getField("rules")).cast("int"))
            .withColumn("expression_count",           F.size(F.col("typology_obj").getField("expression")).cast("int"))
            .withColumn(
                "pk",
                F.sha2(
                    F.concat_ws(
    ...
    def gold(self) -> str:
        ...
        max_rules = s.select(F.max(F.size(F.col("typology_obj").getField("rules"))).alias("m")).collect()[0]["m"] or 0
        max_expr  = s.select(F.max(F.size(F.col("typology_obj").getField("expression"))).alias("m")).collect()[0]["m"] or 0
        max_wghts = (
            s.select(F.explode_outer(F.col("typology_obj").getField("rules")).alias("r"))
             .select(F.max(F.size(F.col("r").getField("wghts"))).alias("m"))
             .collect()[0]["m"] or 0
        )

        wide_cols = []
        for i in range(1, max_rules + 1):
            r = F.element_at(F.col("typology_obj").getField("rules"), i)
            wide_cols += [
        ...
        for k in range(1, max_expr + 1):
            wide_cols.append(
                F.element_at(F.col("typology_obj").getField("expression"), k).cast("string").alias(f"expr_{k:03d}_token")
            )
